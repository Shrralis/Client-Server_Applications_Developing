package ua.kep.javaEELabs.laboratorna_10.logic;
...
public class ManagementSystem {
    ...
    private ManagementSystem() throws Exception {
        try {
            ...
            con = DriverManager.getConnection(url, "root", "zolotorig91");
    ...
    public List<Group> getGroups() throws SQLException {
        ...
        try {
            ...
            rs = stmt
                    .executeQuery("SELECT group_id, groupName, curator, speciality FROM groups");
            while (rs.next()) {
                Group gr = new Group();
    ...
    public Collection<Student> getAllStudents() throws SQLException {
        ...
        try {
            stmt = con.createStatement();
            rs = stmt.executeQuery("SELECT student_id, firstName, patronymic, surName, "
    ...
    public Collection<Student> getStudentsFromGroup(Group group, int year)
        ...
        ResultSet rs = null;
        try {
            stmt = con.prepareStatement("SELECT student_id, firstName, patronymic, surName, "
                    + "sex, dateOfBirth, group_id, educationYear FROM students "
    ...
    public void moveStudentsToGroup(Group oldGroup, int oldYear,
                                    Group newGroup, int newYear) throws SQLException {
        PreparedStatement stmt = null;
        try {
            stmt = con.prepareStatement("UPDATE students SET group_id=?, educationYear=? "
                    + "WHERE group_id=? AND educationYear=?");
